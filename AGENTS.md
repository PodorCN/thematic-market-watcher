...
Codex, Cursor, aider, ...) working in this repo. `CLAUDE.md` is a symlink
to this file — do not fork content into it.
...
1. **Stages talk only through files.** Each stage under `pipeline/` is an
   independently runnable script. It reads its input file(s) from
   `data/<date>/`, writes its output file(s) there, and never imports
...
2. **Date-partitioned archive.** Every day's artifacts live under
   `data/<YYYY-MM-DD>/`. That directory is the full history.
3. **Deterministic code and LLM reasoning are separated**, with one
   deliberate, narrow exception (4a below):
   - Stage 1 (`fetch_data.py`): pure code, no LLM.
   - Stage 2a (`fetch_candidates.py`): LLM call, **with a hosted
     web-search tool** — see 4a.
   - Stage 2b (`judge.py`): LLM call, pure prompt+schema, no tools.
   - Stage 3 (`analyze.py`): LLM call, pure prompt+schema, no tools.
   - Stage 4 (`render.py`): pure code, no LLM.
4. **LLM-provider-independence is a hard requirement:**
   - Pipeline scripts never import a vendor SDK. They only call
     `lib/llm_client.call_llm(...)` or
     `lib/llm_client.call_llm_with_web_search(...)`.
   ...
   - Each provider gets its own small adapter function inside
     `lib/llm_client.py` (`_call_anthropic`, `_call_openai`, ...) because
   ...
   **4a. The one documented exception:** the original design for stage 2
   ...
   owner explicitly overrode that for `fetch_candidates.py`: the LLM
   ...
   general agent loop we hand-roll). `judge.py` immediately downstream of
   it is a plain, tool-free `call_llm()` call, same as `analyze.py` — so
   the "LLM only reasons over prepared data" property still holds
   everywhere except this one retrieval step.
...
config/llm.yaml         # per-stage provider/model
lib/
...
- Add a new market-data source by adding a `MarketDataProvider` subclass
  in `lib/market_data_provider.py` and registering it in `_PROVIDERS` —
  don't touch `fetch_data.py`.
- Add a new LLM provider by adding `_call_<provider>` (and optionally
  `_call_<provider>_with_web_search`) in `lib/llm_client.py` and
  registering it in the two dispatch dicts at the bottom of that file.
- Prompts are plain markdown files under `pipeline/*/`, not embedded in
  any SKILL.md or agent-specific wrapper. If this pipeline is ever
  wrapped as a Claude Code skill, the SKILL.md must reference these files
